...
describe('TwitterDatePipe', () => {
  beforeEach(() => {
    this.pipe = new TwitterDatePipe();
  });

  it('strips out the time and timezone data', () => {
    expect(this.pipe.transform('Tue Dec 23 040333 +0000 2008')).toEqual('Tue Dec 23 2008');
  });

  describe('if date in unexpected format, returns "Not Available"', () => {
    it('date string not 6 words long', () => {
      expect(this.pipe.transform('Dec 23 040333 +0000 2008')).toEqual('Not Available');
    });

    it('forth word is not a time stamp', () => {
      expect(this.pipe.transform('Tue Dec 23 +0000 2008')).toEqual('Not Available');
    });

    it('fifth word is not a timezone', () => {
      expect(this.pipe.transform('Tue Dec 23 04:03:33 2008')).toEqual('Not Available');
    });
  });
});
